...
#define ODD_M 0  // not work in driver
...
// general idea: A*B+C = transpose the result of  transpose(B) * transpose(A)+ transpose(C) 
// B is transposed as default
...
)
{
	...
	const uint32_t base_offset_a =  thread_id_0 * TILE_N * SIZE_K * SIZE_OF_HF16_BYTE;
	const uint32_t base_offset_b =  thread_id_1 * TILE_M * SIZE_K * SIZE_OF_HF16_BYTE;
	const uint32_t base_offset_output =  (thread_id_0 * TILE_N  + thread_id_1 * TILE_M * SIZE_N) * SIZE_OF_HF16_BYTE;
...
#endif
	// init TILE_A
	vector<HALF, TILE_N * TILE_K > readA = 0.0; 	// A tile: (8*TILE_M/8)M x 16K
	...
	matrix<HALF, TILE_M, TILE_K> tileB = 0.0;  
	
	//init the accumulators
	matrix<FLOAT, TILE_N, TILE_M> result1 = 0.0; 
	matrix<FLOAT, TILE_N, TILE_M> result1_last = 0.0;  
	
	for( int step = 0; step < SIZE_K / TILE_K; step++)
	{
		const uint32_t step_base_offset_a = base_offset_a + step * TILE_K * SIZE_OF_HF16_BYTE;
		const uint32_t step_base_offset_b = base_offset_b + step * TILE_K * SIZE_OF_HF16_BYTE;
		
		//cache elements in matrix B 
		#pragma unroll
		for(int row = 0; row < TILE_M; row++)
		{
			const uint32_t read_offset_b = step_base_offset_b + (row * SIZE_K)* SIZE_OF_HF16_BYTE;
			tileB.select<1,1,TILE_K,1>(row,0).format<U32>() = cm_load<U32, TILE_K/2, DataSize::Default, CacheHint::Cached, CacheHint::Cached>(surface_input_b, read_offset_b);  
		}		

		#pragma unroll
		for(int m=0; m < TILE_N/8; m++)
		{
			...
			for(int n = 0; n < TILE_M/8; n++)  
			{
				...
				for(int k = 0; k < TILE_K/16; k++)
				{
					...
					for (int row = 0; row < 8; row++)
					{		
						readB_m.select<8,1,1,1>(0, row*2)= tileB.select<1,1,8,2>(row + n*8, 16*k);
						readB_m.select<8,1,1,1>(0, row*2+1)= tileB.select<1,1,8,2>(row + n*8, 16*k+1);	
					}	
	...
	// {
	// 	printf(" normal_readA: SIZE_N:%d SIZE_K:%d\n",SIZE_N,SIZE_K);
	...
	// 	printf(" normal_readB: SIZE_K:%d SIZE_M:%d\n",SIZE_K,SIZE_M);
	// 	for (int i = 0; i < 8; i++)
	// 	{
	// 		printf(" row%d", i);
	// 		for (int j = 0; j < 16;j++)
	...
	// 	printf(" result: SIZE_N:%d SIZE_M:%d\n",SIZE_M,SIZE_M);
	// 	for (int i = 0; i < TILE_N; i++)
	// 	{
	// 		printf(" row%d", i);
...
#if USE_INPUTC
	matrix<HALF, TILE_N, TILE_M> bias = 0.0; 
	#pragma unroll
	for(int i = 0; i < TILE_M; i++)
	{
		const unsigned write_index = base_offset_output + i * SIZE_N * SIZE_OF_HF16_BYTE;
		bias.select<TILE_N,1, 1, 1>(0,i).format<U32>() = cm_load<U32, TILE_N/2, DataSize::Default, CacheHint::Cached, CacheHint::Cached>(surface_input_c, write_index);
	}
	...
	{
		const unsigned write_index = base_offset_output + i * SIZE_N * SIZE_OF_HF16_BYTE;
		result_hf16_CL1.select<TILE_N, 1>(0)  = result1.select<TILE_N, 1, 1, 1>(0, i);  //read and transpose 
