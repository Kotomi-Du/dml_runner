...
)
{
	...
	// init TILE_A
	vector<HALF, SIZE_PER_DPAS_HF16 * TILE_M / 8> readA = 0.0; 	// M=0..7,  K=0..15		// A tile: (8*TILE_M/8)M x 16K
	...
	/** solution 1 end*/
	vector<HALF, SIZE_PER_DPAS_HF16 * TILE_N / 8 > readB = 0.0; 	// N=0..7,  K=0..15		//B tile: 16Kx8N
	matrix_ref<HALF, 8, TILE_N*2> readB_m = readB.format<HALF, 8, TILE_N*2>();	// even though more registers are used here, the final registers are less than solution 1, and the perf is almost same
	...
	for( int step = 0; step < SIZE_K; step += TILE_K)
	{
		...
		for(int m=0; m < TILE_M/8; m++)
		{
			...
			{
				#if SLM_KN_SHARING
					myDPAS8(readA_m.select<8,1,16,1>(m * 8, 0), readB_m.select<8,1,16,1>(0, n*16),result1ref.select<8,1,8,1>(m * 8,n * 8));
				#else
					#pragma unroll
					for (int row = 0; row < 8; row++)
					{
						readB_m.select<1,1,8,2>(row, n*16)= rowX2_0.select<1,1,8,1>(row,8*n);
						readB_m.select<1,1,8,2>(row, 1 + n*16)= rowX2_1.select<1,1,8,1>(row,8*n);		
					}	
					myDPAS8(readA_m.select<8,1,16,1>(m * 8, 0),  readB_m.select<8,1,16,1>(0, n*16), result1ref.select<8,1,8,1>(m * 8,n * 8));  
				#endif
			}
		...
		// if (thread_id_0 == 4 && thread_id_1 == 0)
		// {
		...
		// if (thread_id_0 == 15 && thread_id_1 == 159)
		// {
		// 	for (int i = 0; i < 8; i++)
    	// 	{
        // 		  printf(" row%d", i);
