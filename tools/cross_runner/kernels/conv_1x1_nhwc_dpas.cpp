...
_GENX_ inline vector<DT_IN, BLOCK_W * DPAS_INPUT_CHANNELS> load_input_nhwc_and_reorder_to_wc16(SurfaceIndex surface [[type("buffer_t")]], uint byte_offset)
{
    ...
    {
        vector<uint32_t, 8> load_chunk = cm_load<uint32_t, 8, DataSize::Default, CacheHint::Cached, CacheHint::Cached>(surface, byte_offset);
        vector<half, DPAS_INPUT_CHANNELS> load_chunk_typed = load_chunk.format<half>();  
        data_out.select<DPAS_INPUT_CHANNELS, 1>(i*DPAS_INPUT_CHANNELS) = load_chunk_typed.select<DPAS_INPUT_CHANNELS, STRIDE_W>();
        byte_offset += INPUT_CHANNELS * sizeof(DT_IN);
    }  
...
_GENX_ inline void store_output_wc8_as_nhwc(SurfaceIndex surface [[type("buffer_t")]], vector_ref<DT_OUT, BLOCK_W * DPAS_OUTPUT_CHANNELS> grf_chunk, uint32_t byte_offset, uint32_t w_chunk_id)
{   
#if !HAS_LEFTOVER 
    #pragma unroll
    for(int i = 0; i < BLOCK_W; i++)
    {
        vector_ref<half, DPAS_OUTPUT_CHANNELS> grf_chunk_store = grf_chunk.select<DPAS_OUTPUT_CHANNELS, 1>(i * DPAS_OUTPUT_CHANNELS);
        cm_store<U32, DPAS_OUTPUT_CHANNELS/2, DataSize::Default, CacheHint::WriteBack, CacheHint::WriteBack>(surface, byte_offset, grf_chunk_store.format<U32>() );
        byte_offset += OUTPUT_CHANNELS * sizeof(DT_OUT);
    }
#endif
}

extern "C" _GENX_MAIN_ void convolution_nhwc_1x1(
...
)
{
    ...
    uint32_t input_offset_bytes = (input_batch_offset + input_h_chunk_offset + input_w_chunk_offset + input_slice_ic_chunk_offset) * sizeof(DT_IN);
    ...
    {
        vector<DT_IN, BLOCK_W * DPAS_INPUT_CHANNELS> input_row_0 = load_input_nhwc_and_reorder_to_wc16<BLOCK_W>(surface_input, input_offset_bytes);
        ...
        if (thg_0 == 0 && h_chunk_id == 0 && thread_id_2==0 )
         {
            printf("\ninput: %d, %d, %d\n", input_offset_bytes/sizeof(DT_IN), STRIDE_W, HAS_LEFTOVER );
...
#endif
        input_offset_bytes += (input_dpas_ic_offset_size * sizeof(DT_IN));
...
#if ACTIVATION_FUNCTION_LEAKY_RELU
   vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_0_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_0;
   output_row_0_oc_0 = cm_max<DT_OUT>(output_row_0_oc_0, output_row_0_oc_0_leaky);
#if BLOCK_OC >= 16
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_1_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_1;
    output_row_0_oc_1 = cm_max<DT_OUT>(output_row_0_oc_1, output_row_0_oc_1_leaky);
#endif

#if BLOCK_OC >= 32
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_2_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_2;
    output_row_0_oc_2 = cm_max<DT_OUT>(output_row_0_oc_2, output_row_0_oc_2_leaky);
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_3_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_3;
    output_row_0_oc_3 = cm_max<DT_OUT>(output_row_0_oc_3, output_row_0_oc_3_leaky);
    
#endif
#if BLOCK_OC >= 40
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_4_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_4;
    output_row_0_oc_4 = cm_max<DT_OUT>(output_row_0_oc_4, output_row_0_oc_4_leaky);
#endif
#if BLOCK_OC >= 64

    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_5_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_5;
    output_row_0_oc_5 = cm_max<DT_OUT>(output_row_0_oc_5, output_row_0_oc_5_leaky);
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_6_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_6;
    output_row_0_oc_6 = cm_max<DT_OUT>(output_row_0_oc_6, output_row_0_oc_6_leaky);
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_7_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_7;
    output_row_0_oc_7 = cm_max<DT_OUT>(output_row_0_oc_7, output_row_0_oc_7_leaky);
#endif
#if BLOCK_OC == 80
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_8_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_8;
    output_row_0_oc_8 = cm_max<DT_OUT>(output_row_0_oc_8, output_row_0_oc_8_leaky);
    vector<DT_OUT, ACCU_REG_SIZE> output_row_0_oc_9_leaky = (half)ACTIVATION_ALPHA * output_row_0_oc_9;
    output_row_0_oc_9 = cm_max<DT_OUT>(output_row_0_oc_9, output_row_0_oc_9_leaky);
#endif
#endif
    ...
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_0, output_offset, w_chunk_id);  
    
#if BLOCK_OC >= 16
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_1, output_offset, w_chunk_id); 
#endif

#if BLOCK_OC >= 32
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_2, output_offset, w_chunk_id); 
    
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_3, output_offset, w_chunk_id); 
#endif

#if BLOCK_OC >= 40
    output_offset += (DPAS_OUTPUT_CHANNELS ) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_4, output_offset, w_chunk_id); 
#endif

#if BLOCK_OC >= 64
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_5, output_offset, w_chunk_id); 
	
	output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_6, output_offset, w_chunk_id); 
    
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_7, output_offset, w_chunk_id);
#endif

#if BLOCK_OC == 80
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_8, output_offset, w_chunk_id); 
    
    output_offset += (DPAS_OUTPUT_CHANNELS) * sizeof(DT_OUT);
    store_output_wc8_as_nhwc<BLOCK_W>(surface_output, output_row_0_oc_9, output_offset, w_chunk_id); 
#endif
}