...
extern "C" _GENX_MAIN_ void weights_reorder(SurfaceIndex surface_input [[type("buffer_t")]], SurfaceIndex surface_output [[type("buffer_t")]])
{
	...
	//printf("thread_id:%d,%d,%d ", thread_id_0, thread_id_1, thread_id_2);
	...
	const uint32_t ic_chunks_per_hw_thread = DPAS_EXEC_SIZE / 2;
...
#if DEBUG_MODE
    if (thread_id_0 == 0 && thread_id_1 == 0 && thread_id_2 == 0)
    {
        printf("\nreorder: \n");
        for (int i = 0; i < chunks_count; i++)
        {
            for (int j = 0; j < ic_per_hw_thread; j++)
            {
                printf("%8.3f ", data_input(i, j));
            }
            printf("\n");
        }
    }
